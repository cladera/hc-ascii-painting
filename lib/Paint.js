...
Paint.prototype.print = function(command){
  ...
  switch (cmd) {
    case PaintingCommands.PAINT_SQUARE:
      this.square_(args);
      break;
...
Paint.prototype.line_ = function(args){
  var i;
  if(args[0] === args[2]) {
...
Paint.prototype.square_ = function(args){
  var r,
    c;
  for(r = args[0] - args[2]; r <= (args[0] + args[2]); r++){
    for(c = args[1] - args[2]; c <= (args[1] + args[2]); c++){
      this.canvas_[r][c] = '#';
    }
  }
};
